    ...
    this.rentalService.rent(this.confirmOrderInputModel.rentPaymentRequest).subscribe(response => {
      if (this.confirmOrderInputModel.isCreditCardSaving === true) {
        this.saveCreditCard().then((result) => {
          result === true
            ? this.toastrService.success("Carte de crédit enregistrée avec succès", "Carte de crédit enregistrée")
            : this.toastrService.warning("Échec de l'enregistrement de la carte de crédit", "Échec de l'enregistrement de la carte de crédit");
        });
      }
      this.toastrService.success(response.message, "Paiement réussi")
      ...
      error => {
        this.toastrService.error(error.error.message, "Paiement échoué")
        this.spinner.hide();
      });
  }
