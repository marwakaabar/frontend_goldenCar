    ...
    if (!this.carAddForm.valid) {
      ...
      this.carService.add(carModel).subscribe(carAddSuccessResponse => {
        if (this.carImagesFiles.length === 0) {  //No pictures, just car added
          this.toastrService.success("Nouvel outil ajouté avec succès", "Opération réussie");
          ...
          if (this.carImagesFiles.length > 5) { //Max 5 Image
            ...
            this.uploadAllImagesToServer(this.carImagesFiles, carAddSuccessResponse.data).then((unUploadFileList) => {
              ...
              if (unUploadedFiles.length === 0) {
                this.toastrService.success("Nouvel outil et images ajoutés avec succès", "Opération réussie");
                this.closeCarAddModal();
              } else {
                ...
                this.toastrService.warning("Le nouvel outil a été ajouté avec succès, mais certaines images n'ont pas pu se charger. Fichiers téléchargeables :" + failFileNameMessage, "Transaction partiellement réussie");
                this.closeCarAddModal();
              }
            })
        ...
        this.errorService.showBackendError(errorResponse, "Échec de l'ajout du véhicule");
      })
    }
  }
  ...
  addCarImagesToCarImagesAndPathList(imageList: any) {
    if (imageList.length !== 0) {
      if (this.carImagesFiles.length < 5) {
        ...
        this.toastrService.error("Vous pouvez ajouter jusqu'à 5 images", "Impossible d'ajouter des images");
      }
    ...
    return new Promise<boolean>((result) => {
      this.checkFileMimeType(image).then((successStatus) => {
        if (successStatus) {
          ...
          this.toastrService.error("Vous ne pouvez télécharger qu'un fichier image", "Aucun fichier joint");
          result(false);
        }
      })
    })
