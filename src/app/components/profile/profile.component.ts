  ...
  updateProfile() {
    if (this.updateProfileForm.valid) {
      ...
      this.userService.updateProfile(user).subscribe(() => {
        ...
        this.toastrService.success("Veuillez vous reconnecter", "Votre profil a été mis à jour avec succès");
      }, errorResponse => {
        this.errorService.showBackendError(errorResponse, "Échec de la mise à jour du profil");
      })
    } else {
      this.toastrService.error("Au moins une des informations saisies est incorrecte", "Information incorrecte")
    }
  }
  ...
  changePassword() {
    if (this.changePasswordForm.valid) {
      ...
      this.authService.changePassword(changePasswordModel).subscribe(() => {
        this.logOutAndGoLoginPage();
        this.toastrService.success("Veuillez vous reconnecter", "Votre mot de passe a été mis à jour avec succès");
      }, errorResponse => {
        this.errorService.showBackendError(errorResponse, "Impossible de mettre à jour le mot de passe");
      });
    } else {
      this.toastrService.error("Au moins un des mots de passe saisis n'est pas valide", "Informations incorrectes")
    }
  }
